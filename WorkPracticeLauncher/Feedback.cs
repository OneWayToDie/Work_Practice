...
namespace WorkPracticeLauncher
{
	...
	public static class FeedbackManager
	{
		...
		private static string ExtractJsonValue(string json, string key)
		{
			...
			int i = start;
			while (i < json.Length)                                    // Перебираем символы
			{
				if (json[i] == '\\' && i + 1 < json.Length)           // Если обратный слэш
					i += 2;                                           // Пропускаем экранированный символ
				else if (json[i] == '"')                               // Если незакрытая кавычка
					break;                                            // Конец значения
				else
					i++;                                              // Следующий символ
			}
			if (i >= json.Length) return "";                           // Если не нашли закрытие
			string raw = json.Substring(start, i - start);            // Извлекаем сырое значение
			return raw.Replace("\\\"", "\"")                           // Декодируем экранированные кавычки
					 .Replace("\\n", "\n")                           // Декодируем перенос строки
					 .Replace("\\\\", "\\");                         // Декодируем обратный слэш
		}
